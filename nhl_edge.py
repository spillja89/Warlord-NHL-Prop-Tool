...
def build_tracker(today_local: date, debug: bool = False) -> str:
    ...
    def _latest_gamelog_date(payload: Optional[Dict[str, Any]]) -> Optional[date]:
        """Best-effort: newest gameDate in player's completed gameLog."""
        ...
        for r in gl:
            if not isinstance(r, dict):
                continue
            s = str(r.get("gameDate") or r.get("gameDateUTC") or r.get("startTimeUTC") or "").strip()
            if not s:
                continue
            try:
    ...
    # If a cached payload is stale (missed last night's game), refresh it.
    # This fixes 'assist yesterday but drought shows 1' when the cache was created before the game finished.
    STALE_DAYS = 2  # refresh if newest gameDate is >=2 days behind today_local

    def fetch_one(pid: int) -> Tuple[int, Optional[Dict[str, Any]]]:
        key = str(pid)
        if key in cache:
            cached = cache.get(key)
            latest = _latest_gamelog_date(cached)
            try:
                age_days = (today_local - latest).days if latest is not None else 999
            except Exception:
                age_days = 999
