...
def lock_badge(green_bool, ev_icon) -> str:
    """🔒 when a market is BOTH earned-green and +EV (the strongest signal)."""
    try:
        g = bool(green_bool)
    except Exception:
        g = False
...
if page == "Board":
    ...
    if "Any_Lock" not in df_b.columns:
        df_b["Any_Lock"] = (
            (df_b.get("Green_Points", False).fillna(False).astype(bool) & (df_b.get("Plays_EV_Points", "").astype(str).str.strip() == "💰"))
            | (df_b.get("Green_SOG", False).fillna(False).astype(bool) & (df_b.get("Plays_EV_SOG", "").astype(str).str.strip() == "💰"))
            | (df_b.get("Green_Goal", False).fillna(False).astype(bool) & (df_b.get("Plays_EV_ATG", "").astype(str).str.strip() == "💰"))
            | (df_b.get("Green_Assists", False).fillna(False).astype(bool) & (df_b.get("Plays_EV_Assists", "").astype(str).str.strip() == "💰"))
        )
        df_b["LOCK"] = df_b["Any_Lock"].map(lambda x: "🔒" if bool(x) else "")
...
# =========================
    ...
    points_cols = [
        ...
        "Matrix_Points","Conf_Points","Green","Plays_EV_Points","LOCK","💰","Points_Call","GF_Gate_Badge","Tier_Tag",
        # --- EV / Odds ---
        "Points_Line",
        "Points_Book",
        "Points_Odds_Over",
...
# =========================
    ...
    df_a["LOCK"] = df_a.apply(lambda r: lock_badge(r.get("Green_Assists", False), r.get("Plays_EV_Assists", "")), axis=1)

    assists_cols = [
        "Game",
        "Player", "Pos", "Team", "Opp","Markets", 
        "Matrix_Assists",
        "Conf_Assists", "Green","Plays_EV_Assists","LOCK","💰","Assists_Call","GF_Gate_Badge", "Tier_Tag","Drought_A","Best_Drought",
...
# =========================
    ...
    sog_cols = [
       ...
       "Player", "Pos", "Team", "Opp","Markets",
       "Matrix_SOG",
       "Conf_SOG", "Green", "Plays_EV_SOG","LOCK","💰","SOG_Call", "Tier_Tag", "Drought_SOG", "Best_Drought",
...
# =========================
    ...
    df_g["LOCK"] = df_g.apply(lambda r: lock_badge(r.get("Green_Goal", False), r.get("Plays_EV_ATG", "")), axis=1)

    goal_cols = [
        "Game",
        "Player", "Pos", "Team", "Opp","Markets",
        "Matrix_Goal",
        "Conf_Goal", "Green", "Plays_EV_ATG","LOCK","💰","ATG_Call", "GF_Gate_Badge", "Tier_Tag", "💰",
        "ATG_Line", "ATG_Book", "ATG_Odds_Over",
        "ATG_Model%", "ATG_Imp%", "ATG_EV%",
