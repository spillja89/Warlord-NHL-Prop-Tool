...
__BDL_EV_ENGINE__ = "ATG_POISSON_v1_2026-01-18"
...
def add_bdl_ev_all(tracker: pd.DataFrame) -> pd.DataFrame:
    ...
    for out_prefix, cfg in cfgs.items():
        ...
        df[f"{out_prefix}_ModelSrc"] = pd.Series(p_model_src).astype("string")
        df[f"{out_prefix}_p_model_over"] = pd.to_numeric(pd.Series(p_model), errors="coerce")
        ...
        df[f"{out_prefix}_EVpct_over"] = pd.to_numeric(pd.Series(evs), errors="coerce")
