...
"
    ...
    def _is_moneybag(v) -> bool:
        return str(v).strip() == "💰"

    def _col_as_bool(df: pd.DataFrame, col: str) -> pd.Series:
    ...
    def _col_as_money(df: pd.DataFrame, col: str) -> pd.Series:
        if col not in df.columns:
            return pd.Series(False, index=df.index)
        return df[col].astype(str).apply(_is_moneybag)

    def apply_market_filters(df_in: pd.DataFrame, filters: dict, green_col: str, ev_icon_col: str,
                             conf_col: str | None = None, matrix_col: str | None = None,
                             lock_col: str = "LOCK") -> pd.DataFrame:
        df = df_in.copy()
        # conf gate
        if conf_col and conf_col in df.columns:
            df = df[pd.to_numeric(df[conf_col], errors="coerce").fillna(0) >= float(filters.get("min_conf", 0))]
        # hide red rows
        if filters.get("hide_reds") and matrix_col and matrix_col in df.columns:
            df = df[~df[matrix_col].astype(str).str.lower().str.contains("red", na=False)]
        # greens only
        if filters.get("greens_only"):
            g = _col_as_bool(df, green_col)
            df = df[g]
        # ev only
        if filters.get("ev_only"):
            e = _col_as_money(df, ev_icon_col)
            df = df[e]
        # locks only
        if filters.get("locks_only"):
            if lock_col in df.columns:
                ...
                df = df[g & e]
        # plays first sorting
        if filters.get("plays_first"):
            sort_cols = []
            if lock_col in df.columns: sort_cols.append(lock_col)
            if ev_icon_col in df.columns: sort_cols.append(ev_icon_col)
            if conf_col and conf_col in df.columns: sort_cols.append(conf_col)
            # convert icons for sorting
            tmp = df.copy()
            if lock_col in tmp.columns:
                tmp["_lock_sort"] = (tmp[lock_col].astype(str).str.strip() == "🔒").astype(int)
                sort_cols = ["_lock_sort"] + [c for c in sort_cols if c != lock_col]
            if ev_icon_col in tmp.columns:
                tmp["_ev_sort"] = (tmp[ev_icon_col].astype(str).str.strip() == "💰").astype(int)
                sort_cols = ["_ev_sort"] + [c for c in sort_cols if c != ev_icon_col]
            # build ascending flags
            asc = [False] * len(sort_cols)
            for i,c in enumerate(sort_cols):
                if c in [conf_col]:
                    asc[i] = False
            tmp = tmp.sort_values(by=sort_cols, ascending=asc, kind="mergesort")
...
page = st.sidebar.radio(
    "Page",
    ["Board", "Points", "Assists", "SOG", "GOAL (1+)","Guide", "Ledger", "Raw CSV", "📟 Calculator"],
    index=0
)
...
# =========================
    ...
    st.markdown("Use this to size bets with discipline. **Signals tell you what to play. This tells you how much.**")
    legend_signals()
    ...
    with c3:
        max_pct = st.slider("Max Stake (% of bankroll)", 0.0, 0.20, 0.05, 0.01)

    st.markdown("### Inputs")
    c1, c2, c3, c4 = st.columns([1.2,1.0,1.0,1.0])
    with c1:
        player = st.text_input("Player (optional)", value="")
    with c2:
        market = st.selectbox("Market", ["Points","SOG","Goal","Assists"], index=0)
    with c3:
        line = st.text_input("Line (e.g. 0.5 / 2.5)", value="")
    with c4:
        american_odds = st.number_input("Odds (American)", value=-110, step=5)

    c1, c2, c3 = st.columns([1,1,1])
    with c1:
        model_prob = st.slider("Model win probability (%)", 1.0, 99.0, 55.0, 0.5) / 100.0
    with c2:
        manual_ev = st.number_input("EV% (optional override)", value=0.0, step=0.5)
    with c3:
        use_manual_ev = st.checkbox("Use manual EV% instead of computed", value=False)
    ...
    imp = implied_prob(float(american_odds))

    # EV% computed: expected profit per $1 stake
    b = dec - 1.0
    p = float(model_prob)
    q = 1.0 - p
    ev_per_dollar = (p * b) - q
    ...
    stake_cap = bankroll * float(max_pct)
    ...
    c1, c2, c3 = st.columns([1,1,1])
    c1.metric("Kelly % (full)", f"{kelly*100:.2f}%")
    c2.metric("Recommended Stake ($)", f"{stake:.2f}")
    c3.metric("Stake % of Bankroll", f"{(stake/bankroll*100.0) if bankroll>0 else 0.0:.2f}%")

    st.caption("Tip: If EV is small, keep stakes small. If you're not sure, start conservative (0.25 Kelly + 5% cap).")
