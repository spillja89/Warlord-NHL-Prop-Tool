...
def add_bdl_ev_all(tracker: pd.DataFrame) -> pd.DataFrame:
    ...
    for out_prefix, cfg in cfgs.items():
        ...
        # If we have odds but no line (BDL coverage sometimes returns odds with missing/failed line merge),
        # force baseline milestone lines so Model% and EV can still compute.
        if out_prefix in {"Points","Assists","Goal","ATG"}:
