...
\
    ...
    with c3:
        max_pct = st.slider("Max Stake cap (% bankroll)", 0.0, 0.20, 0.05, 0.01)
    with c4:
        odds_txt = st.text_input("Odds (American, e.g. -110 / +145)", value="-110")

    model_prob = st.slider("Model win probability (%)", 1.0, 99.0, 55.0, 0.5) / 100.0

    def parse_american(x: str) -> float:
    ...
    try:
        american_odds = parse_american(odds_txt)
        dec = american_to_decimal(american_odds)
        imp = implied_prob(american_odds)
        ...
        st.caption("Rule: best bets are **🟢 + 💰** together. Use small stakes when EV is small; scale with discipline.")
