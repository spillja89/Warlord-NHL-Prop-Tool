...
# ============================
# SportsDataIO (optional) — player cache
# ============================

SPORTSDATAIO_BASE = "https://api.sportsdata.io/v3/nhl/scores/json"
SPORTSDATAIO_TIMEOUT = 20
SPORTSDATAIO_CACHE_DIR = "cache"
SPORTSDATAIO_PLAYERS_CACHE = os.path.join(
    SPORTSDATAIO_CACHE_DIR,
    "sportsdataio_players.json",
)
SPORTSDATAIO_PLAYERS_TTL_HOURS = 72  # refresh every 3 days
...
def _file_is_fresh(path: str, ttl_hours: int) -> bool:
    try:
...
def fetch_sportsdataio_players(
    force: bool = False,
    verbose: bool = False,
) -> list[dict]:
    """
    Fetch /Players from SportsDataIO.
    Cached to cache/sportsdataio_players.json.
    """
    _ensure_dir(SPORTSDATAIO_CACHE_DIR)

    if (
        not force
        and os.path.exists(SPORTSDATAIO_PLAYERS_CACHE)
        and _file_is_fresh(
            SPORTSDATAIO_PLAYERS_CACHE,
            SPORTSDATAIO_PLAYERS_TTL_HOURS,
        )
    ):
        try:
            with open(
    ...
    if not key:
        if verbose:
            print("[sportsdataio] SPORTSDATAIO_KEY missing — skipping")
        return []

    try:
        import requests

        url = f"{SPORTSDATAIO_BASE}/Players"
        r = requests.get(
            url,
            headers={"Ocp-Apim-Subscription-Key": key},
            timeout=SPORTSDATAIO_TIMEOUT,
        )

        if not r.ok:
            if verbose:
                print(
                    f"[sportsdataio] HTTP {r.status_code}: "
                    f"{r.text[:200]}"
                )
            return []
...
def build_sportsdataio_player_index(players: list[dict]) -> dict[str, dict]:
    idx: dict[str, dict] = {}
    ...
    for p in players or []:
        ...
        k = norm_name_key(full)

        if not k:
            continue

        idx[k] = p
        idx[k.replace("'", "")] = p  # O'Reilly fallback
...
def _is_completed_game_row(r: dict) -> bool:
    """
    NHL game-log rows sometimes include future scheduled games with empty/zero stats.
    Those will break drought logic because we sort by date and treat them as 'most recent'.

    Keep only games that look completed / have real recorded stats.
    """
    # 1) If gameState exists, prefer it
    st = str(r.get("gameState", "") or "").upper()
    if st in {"OFF", "FINAL", "FINAL_OT", "FINAL_SO"}:
        return True
    if st in {"FUT", "PRE", "LIVE", "CRIT"}:
        return False

    # 2) If we can detect stats dict, require at least one meaningful stat field
    stats = None
    ...
    # If no nested stats dict, fall back to top-level
    cand = stats if isinstance(stats, dict) else r

    # Completed rows almost always have at least one of these keys with a non-null value
    keys = ("goals", "assists", "shotsOnGoal", "shots", "timeOnIce", "toi")
    for k in keys:
        if k in cand:
            val = cand.get(k)
            if val is None:
                continue
            # timeOnIce might be "00:00" for scheduled games
            if isinstance(val, str) and k in {"timeOnIce", "toi"}:
                if val.strip() not in {"", "00:00", "0:00"}:
                    return True
            else:
                try:
                    if float(val) > 0:
                    ...
                    # If it's present but not parseable, still treat as "exists"
                    return True

    # If we couldn't prove it has recorded stats, assume it's not completed
    return False
...
def compute_lastN_features(payload: Dict[str, Any], n10: int = 10, n5: int = 5, debug: bool = False) -> Dict[str, Any]:
    rows = _extract_game_rows(payload)

    # ✅ keep only completed games (prevents future scheduled rows breaking drought)
    rows = [r for r in rows if _is_completed_game_row(r)]


    # --- HARD DEBUG for Eichel (prints no matter what payload looks like) ---
    if debug:
        nm = str(payload.get("playerName") or payload.get("name") or payload.get("fullName") or "")
        pid = str(payload.get("playerId") or payload.get("id") or payload.get("player_id") or "")

        if ("eichel" in nm.lower()) or (pid in {"8478402"}):
            print("\n=== DEBUG: EICHEL compute_lastN_features() ===")
            print("payload name:", nm, "| payload id:", pid)
            print("raw rows count:", len(rows))

            for i, r in enumerate(rows[:3]):
                print(f"\n-- row[{i}] keys:", list(r.keys())[:40])
                for k in ["gameDate", "startTimeUTC", "gameState", "playerGameStats", "stats", "skaterStats"]:
                    if k in r:
                        print(f"row[{i}].{k} =", r.get(k))


    # MOST RECENT first
    rows = sorted(
        rows,
        key=lambda r: (r.get("gameDate") or r.get("startTimeUTC") or r.get("game_date") or ""),
        reverse=True
    )

    # Build last-10 lists using robust SOG picker
    v10_rows = rows[:n10]
    v10_goals: List[int] = []
    v10_assists: List[int] = []
    v10_shots: List[int] = []

    for r in v10_rows:
        g = _pick_stat(r, ("goals", "g"))
        a = _pick_stat(r, ("assists", "a"))
        s = pick_sog_from_row(r)  # ✅ IMPORTANT: use robust SOG picker

        v10_goals.append(int(g) if g is not None else 0)
        v10_assists.append(int(a) if a is not None else 0)
        v10_shots.append(int(s) if s is not None else 0)

    # last-5 are just first 5 of last-10 (because already most-recent-first)
    v5_goals = v10_goals[:n5]
    v5_assists = v10_assists[:n5]
    v5_shots = v10_shots[:n5]
    ...
    def drought_from_most_recent(vals: List[int], min_val: int = 1) -> int:
        d = 0
        for v in vals:  # vals[0] is MOST RECENT
            if int(v) >= min_val:
                break
            d += 1
        return d
    ...
    g5_total = int(sum(v5_goals)) if v5_goals else None
    a5_total = int(sum(v5_assists)) if v5_assists else None
    a10_total = int(sum(v10_assists)) if v10_assists else None

    p10 = [g + a for g, a in zip(v10_goals, v10_assists)]
    p10_total = int(sum(p10)) if p10 else None
    g10_total = int(sum(v10_goals)) if v10_goals else None
    s10_total = int(sum(v10_shots)) if v10_shots else None

    drought_p = drought_from_most_recent(p10, 1)
    drought_a = drought_from_most_recent(v10_assists, 1)
    drought_g = drought_from_most_recent(v10_goals, 1)
    drought_sog2 = drought_from_most_recent(v10_shots, 2)
    drought_sog3 = drought_from_most_recent(v10_shots, 3)
...
def heat_from_gap(gap10: Optional[float]) -> str:
    try:
        if gap10 is None or (isinstance(gap10, float) and math.isnan(gap10)):
            return "COOL"
        g = float(gap10)
    except Exception:
        return "COOL"
...
def build_tracker(today_local: date, debug: bool = False) -> str:
    ...
    # -------------------------
    # SportsDataIO — load + index
    # -------------------------
    sdio_players = fetch_sportsdataio_players(
        force=False,
        verbose=debug,
    )
    sdio_idx = build_sportsdataio_player_index(
        sdio_players
    )

    if debug:
        print(f"[sportsdataio] players raw: {len(sdio_players)}")
        print(f"[sportsdataio] index keys: {len(sdio_idx)}")

        # show what SDIO keys look like
        sample_keys = list(sdio_idx.keys())[:10]
        print("[sportsdataio] sample keys:", sample_keys)

        # show what your skater names look like (later, after sk exists)
    ...
    # -------------------------
    # HARD filter: only slate teams
    # -------------------------
    if "Team" in sk.columns and teams_playing:
        before = len(sk)
        ...
        if debug:
            print(
                f"[slate] skaters filtered by Team: "
                f"{before} -> {len(sk)}"
            )
    ...
    def _sd_get(
        ...
        rec = sdio_idx.get(norm_name_key(name))
        return default if not rec else rec.get(field, default)

    sk["SD_PlayerID"] = sk["Player"].apply(
        lambda n: _sd_get(n, "PlayerID")
    )
    sk["Shoots"] = sk["Player"].apply(
        lambda n: _sd_get(n, "Shoots", "")
    )
    sk["Position_SD"] = sk["Player"].apply(
        lambda n: _sd_get(n, "Position", "")
    )

    sk["SD_InjuryStatus"] = sk["Player"].apply(lambda n: _sd_get(n, "InjuryStatus", ""))
    sk["SD_Status"] = sk["Player"].apply(lambda n: _sd_get(n, "Status", ""))
    sk["SD_InjuryNotes"] = sk["Player"].apply(lambda n: _sd_get(n, "InjuryNotes", ""))


    if debug:
        matched = sk["SD_PlayerID"].notna().sum()
        total = len(sk)
        pct = (matched / total * 100.0) if total else 0.0
        print(
            f"[sportsdataio] matched "
            f"{matched}/{total} "
            f"({pct:.1f}%)"
        )
    ...
    try:
        ...
        if debug:
            print("[5v5 player] normalize/merge failed:", e)
        for c in ["i5v5_points60", "i5v5_primaryAssists60", "i5v5_shotAssists60", "i5v5_iCF60"]:
            sk[c] = np.nan

    # Assist volume proxy (bonus-only signal in conf_assists)
    sk["Assist_Volume"] = (
    ...
    if sk["opp_5v5_SlotSA60"].isna().mean() >= 0.99 and debug:
    ...
    # -------------------------
    # Injury source resolution (DFO preferred, SDIO fallback)
    # -------------------------

    # DFO outputs from apply_injury_dfo()
    dfo = (
        sk.get("Injury_Status", pd.Series("", index=sk.index))
          .fillna("")
          .astype(str)
          .str.strip()
    )

    # SportsDataIO fallback fields (if present)
    sdio = (
        sk.get("SD_InjuryStatus", pd.Series("", index=sk.index))
          .fillna("")
          .astype(str)
          .str.strip()
    )
    ...
    sk["Injury_Source"] = np.where(
        (dfo != "") & (dfo.str.upper() != "HEALTHY"),
        "DFO",
        np.where(sdio != "", "SDIO", "")
    )

    # Final resolved injury status
    sk["Injury_Status_Final"] = np.where(
        (dfo != "") & (dfo.str.upper() != "HEALTHY"),
        dfo,
        sdio
    )
    ...
    # Hard exclusion flag
    sk["Injury_HardOut"] = (
        sk["Injury_Status_Final"]
          .astype(str)
          .str.upper()
          .str.strip()
          .isin(["OUT", "IR", "IR-LT", "SUSPENDED", "SCRATCH"])
    )
    ...
    sk["Available"] = ~sk["Injury_HardOut"]

    sk["Unavailable_Reason"] = np.where(
        sk["Available"],
        "",
        sk["Injury_Status_Final"]
    )
    ...
    # -------------------------
    # Regression expectations (TalentMult + market mults BEFORE gaps/heats)
    # -------------------------
    gp = pd.to_numeric(sk["games_played"], errors="coerce").replace(0, np.nan)

    exp_p10 = (
        (pd.to_numeric(sk["iXG_raw"], errors="coerce").fillna(0) +
         pd.to_numeric(sk["iXA_raw"], errors="coerce").fillna(0))
        / gp
    ) * 10.0
    ...
    # global talent prior
    tm = pd.to_numeric(sk.get("TalentMult", 1.0), errors="coerce").fillna(1.0)
    exp_p10 = exp_p10 * tm
    exp_g10 = exp_g10 * tm
    exp_a10 = exp_a10 * tm

    # Base expectations (ONLY global talent prior here)
    sk["Exp_P_10"] = exp_p10.round(2)
    sk["Exp_G_10"] = exp_g10.round(2)
    sk["Exp_A_10"] = exp_a10.round(2)

    # -------------------------
    # Regression gaps + heats (P/G/A/S)
    # Must exist BEFORE matrices/confidence and BEFORE tracker CSV
    # -------------------------

    # Canonical L10 totals (from feats)
    sk["L10_P"] = pd.to_numeric(sk.get("P10_total"), errors="coerce")
    sk["L10_G"] = pd.to_numeric(sk.get("G10_total"), errors="coerce")
    sk["L10_A"] = pd.to_numeric(sk.get("A10_total"), errors="coerce")

    # Canonical L10 shots (already handled in your SOG contract, but keep safe fallback)
    if "L10_S" not in sk.columns:
        sk["L10_S"] = pd.to_numeric(sk.get("S10_total"), errors="coerce")

    # Gaps (positive = due)
    sk["Reg_Gap_P10"] = (pd.to_numeric(sk.get("Exp_P_10"), errors="coerce") - sk["L10_P"]).round(2)
    sk["Reg_Gap_G10"] = (pd.to_numeric(sk.get("Exp_G_10"), errors="coerce") - sk["L10_G"]).round(2)
    sk["Reg_Gap_A10"] = (pd.to_numeric(sk.get("Exp_A_10"), errors="coerce") - sk["L10_A"]).round(2)

    # SOG gap: use your existing Reg_Gap_S10 if present, else compute
    if "Reg_Gap_S10" not in sk.columns:
        sk["Reg_Gap_S10"] = (pd.to_numeric(sk.get("Exp_S_10"), errors="coerce") - sk["L10_S"]).round(2)


    # -------------------------
    # SOG regression CONTRACT (unit-safe + drift-proof)
    # -------------------------

    # Canonical L10 shots column
    if "S10_total" in sk.columns:
        sk["L10_S"] = pd.to_numeric(sk["S10_total"], errors="coerce")
    elif "L10_SOG" in sk.columns:
        sk["L10_S"] = pd.to_numeric(sk["L10_SOG"], errors="coerce")
    else:
        sk["L10_S"] = pd.to_numeric(sk.get("L10_S", np.nan), errors="coerce")

    # ShotIntent raw (could be bad units)
    si_raw = pd.to_numeric(sk.get("ShotIntent", np.nan), errors="coerce")

    # Aliases for median10 (ALWAYS Series)
    med10_pg = pd.to_numeric(
        sk.get("Med10_SOG", pd.Series(np.nan, index=sk.index)),
        errors="coerce"
    )
    if med10_pg.isna().all():
        med10_pg = pd.to_numeric(
            sk.get("Median10_SOG", pd.Series(np.nan, index=sk.index)),
            errors="coerce"
        )

    # Avg5 alias (ALWAYS Series)
    avg5_pg = pd.to_numeric(
        sk.get("Avg5_SOG", pd.Series(np.nan, index=sk.index)),
        errors="coerce"
    )


    # Per-game fallback built from real volume
    fallback_pg = 0.65 * med10_pg + 0.35 * avg5_pg

    # Detect bad units (percentiles / rate60 / NaN / nonsense)
    bad_units = (si_raw > 10) | (si_raw < 0) | si_raw.isna()

    # Final per-game ShotIntent used for math
    sk["ShotIntent_pg"] = np.where(bad_units, fallback_pg, si_raw)
    sk["ShotIntent_pg"] = pd.to_numeric(sk["ShotIntent_pg"], errors="coerce")

    # ✅ Expected shots over 10 (APPLY MARKET MULTIPLIER)
    sk["Exp_S_10"] = (sk["ShotIntent_pg"] * 10.0).round(2)

    # ✅ Regression gap (positive = due)
    ...
    # -------------------------
    # VALIDATION: prevent false SOG output
    # -------------------------
    chk = (sk["Exp_S_10"] - sk["L10_S"]).round(2)
    bad = sk["Reg_Gap_S10"].round(2) != chk

    bad = (
        bad
        & sk["Exp_S_10"].notna()
        & sk["L10_S"].notna()
        & sk["Reg_Gap_S10"].notna()
    )
    ...
    if bad.any():
        ...
        raise ValueError(
            "SOG regression identity violated "
            "(Reg_Gap_S10 must equal Exp_S_10 - L10_S).\n\n"
    ...
    tracker = pd.DataFrame({
        ...
        # --- Last-N totals (explicit for CSV sanity) ---
        "L10_SOG": pd.to_numeric(sk.get("S10_total"), errors="coerce"),
        "L10_G":   pd.to_numeric(sk.get("G10_total"), errors="coerce"),
        "L10_A":   pd.to_numeric(sk.get("A10_total"), errors="coerce"),
        "L10_P":   pd.to_numeric(sk.get("P10_total"), errors="coerce"),
        "L5_G":    pd.to_numeric(sk.get("G5_total"), errors="coerce"),
        "L5_A":    pd.to_numeric(sk.get("A5_total"), errors="coerce"),
        "N_games_found": pd.to_numeric(sk.get("N_games_found"), errors="coerce"),

        "Player": sk["Player"].fillna(""),
        "Team": sk["Team"].fillna(""),
        "Pos": sk["Pos"].fillna("F"),
        "Tier": sk.get("Tier_Tag", ""),
        "SD_PlayerID": sk.get("SD_PlayerID"),
        "Shoots": sk.get("Shoots"),
        "Position_SD": sk.get("Position_SD"),
        ...
        "Injury_Source": sk.get("Injury_Source"),
        "Injury_Status_Final": sk.get("Injury_Status_Final"),
        "SD_InjuryStatus": sk.get("SD_InjuryStatus"),
        "SD_InjuryNotes": sk.get("SD_InjuryNotes"),

        



        "Best_Drought_Market": sk.get("Best_Drought_Market"),
        "Best_Drought_Count": sk.get("Best_Drought"),
        "Best_Drought_Tag": sk.get("Best_Drought_Tag"),
    ...
    # -------------------------
    # Best_Drought display (based on LONGEST drought market, not Best_Market)
    # -------------------------
    def _fmt_int(v: Any) -> str:
        try:
            if v is None or (isinstance(v, float) and math.isnan(v)):
    ...
    def best_drought_display(row: pd.Series) -> str:
        tier = str(row.get("Talent_Tier", "")).upper()

        # TRUE longest drought market you computed earlier
        bm = str(row.get("Best_Drought_Market", "")).upper()
        bd = row.get("Best_Drought_Count", None)

        tags: list[str] = []

        # Always surface POINTS drought for STAR / ELITE (your rule)
        if tier in {"ELITE", "STAR"}:
            dp = _fmt_int(row.get("Drought_P"))
            if dp and int(dp) >= 2:
                tags.append(f"🅿️ PTS D:{dp}")

        # Now show the LONGEST drought market tag
        dstr = _fmt_int(bd)
        if dstr and int(dstr) > 0:
            if bm == "ASSISTS":
                tags.append(f"🅰️ D:{dstr}")
            elif bm == "POINTS":
                tags.append(f"🅿️ D:{dstr}")
            elif bm == "GOAL":
                tags.append(f"🥅 D:{dstr}")
            elif bm == "SOG":
                tags.append(f"🎯 D:{dstr}")
    ...
    # Put display into its own column so we never overwrite numeric drought fields
    tracker["Best_Drought_Display"] = tracker.apply(best_drought_display, axis=1)
    ...
    for c in [
        ...
        "i5v5_primaryAssists60", "Reg_Heat_A", "Reg_Gap_A10", "Drought_A"
    ]:
        if c in tracker.columns:
            tracker[c] = pd.to_numeric(tracker[c], errors="coerce")
    ...
    proof_reg = (
        tracker["Reg_Heat_A"].astype(str).str.upper().isin(["HOT", "WARM"]) |
        (tracker["Reg_Gap_A10"] >= 1.5) |
        (tracker["Drought_A"] >= 2)
    )

    proofs = pd.concat([proof_ixA, proof_v2, proof_team, proof_vol, proof_reg], axis=1).fillna(False)
    tracker["Assist_ProofCount"] = proofs.sum(axis=1)

    conf_gate_ast = (tracker["Conf_Assists"] >= 77)
    ast_earned = conf_gate_ast & (tracker["Assist_ProofCount"] >= 3)
    tracker.loc[ast_earned, "Plays_Assists"] = True

    def _assist_why(r):
        reasons = []
        if float(r.get("iXA%", 0) or 0) >= 92: reasons.append("iXA")
        if float(r.get("v2_player_stability", 0) or 0) >= 65: reasons.append("V2")
        if float(r.get("team_5v5_xGF60_pct", 0) or 0) >= 60: reasons.append("TEAM")
        if (float(r.get("Assist_Volume", 0) or 0) >= 6) or (float(r.get("i5v5_primaryAssists60", 0) or 0) >= 0.45):
            reasons.append("VOL")
        if (str(r.get("Reg_Heat_A", "")).upper() in {"HOT", "WARM"}) or (float(r.get("Reg_Gap_A10", 0) or 0) >= 1.5) or (float(r.get("Drought_A", 0) or 0) >= 2):
            reasons.append("REG")
        return ",".join(reasons)

    tracker.loc[ast_earned, "Assist_Why"] = tracker.loc[ast_earned].apply(_assist_why, axis=1)

    mask = ast_earned & ~tracker["Play_Tag"].fillna("").str.contains("ASSISTS EARNED", regex=False)
    tracker.loc[mask, "Play_Tag"] = np.where(
    ...
    # SOG earned rule
    # -------------------------
    for c in ["Conf_SOG", "ShotIntent_Pct", "Med10_SOG", "Avg5_SOG", "Reg_Heat_S", "Reg_Gap_S10", "Drought_SOG", "team_5v5_SF60_pct"]:
        if c in tracker.columns:
            tracker[c] = pd.to_numeric(tracker[c], errors="coerce")
    ...
    # Proof 1: shot intent profile
    proof_si = (tracker["ShotIntent_Pct"] >= 95)

    # Proof 2: regression / drought timing
    proof_reg = (
        tracker["Reg_Heat_S"].astype(str).str.upper().isin(["HOT", "WARM"]) |
        (tracker["Reg_Gap_S10"] >= 1.5) |
        (tracker["Drought_SOG"] >= 2)
    )

    # Proof 3: volume floor
    proof_vol = (
        (tracker["Med10_SOG"] >= 3.5) |
        (tracker["Avg5_SOG"] >= 3.5)
    )

    # Proof 4: team shot environment
    proof_team = (tracker["team_5v5_SF60_pct"] >= 60)

    proofs = pd.concat([proof_si, proof_reg, proof_vol, proof_team], axis=1).fillna(False)
    tracker["SOG_ProofCount"] = proofs.sum(axis=1)

    conf_gate_sog = (tracker["Conf_SOG"] >= 77)
    sog_earned = conf_gate_sog & (tracker["SOG_ProofCount"] >= 3)
    ...
    def _sog_why(r):
        reasons = []
        if float(r.get("ShotIntent_Pct", 0) or 0) >= 95:
            reasons.append("SI")
        if (
            str(r.get("Reg_Heat_S", "")).upper() in {"HOT", "WARM"} or
            float(r.get("Reg_Gap_S10", 0) or 0) >= 1.5 or
            float(r.get("Drought_SOG", 0) or 0) >= 2
        ):
            reasons.append("REG")
        if (
            float(r.get("Med10_SOG", 0) or 0) >= 3.5 or
            float(r.get("Avg5_SOG", 0) or 0) >= 3.5
        ):
            reasons.append("VOL")
        if float(r.get("team_5v5_SF60_pct", 0) or 0) >= 60:
            reasons.append("TEAM")
        return ",".join(reasons)
    ...
    # -------------------------
    # Best_Conf numeric safety
    # -------------------------
    for c in ["Conf_Points", "Conf_SOG", "Conf_Goal", "Conf_Assists", "Best_Conf"]:
        if c in tracker.columns:
            tracker[c] = pd.to_numeric(tracker[c], errors="coerce").fillna(0).astype(int)
    ...
    out_csv = os.path.join(OUTPUT_DIR, f"nhl_edge_tracker_{today_local.isoformat()}.csv")
    tracker.to_csv(out_csv, index=False)

    # Also save a "latest" file for your Streamlit app to pick up
    latest_csv = os.path.join(OUTPUT_DIR, "nhl_edge_tracker_latest.csv")
    tracker.to_csv(latest_csv, index=False)
...
def main():
    ...
    if args.date:
        today_local = date.fromisoformat(args.date.strip())
    else:
        today_local = datetime.now(ZoneInfo(LOCAL_TZ)).date()

    build_tracker(today_local=today_local, debug=bool(args.debug))
